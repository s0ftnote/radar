...
test("用户可以创建、区分、重新打开并在重启后保留 Radar Brief", async ({ browser }) => {
  const dataDirectory = await mkdtemp(join(tmpdir(), "radar-briefs-"));
  ...
  try {
    await page.goto("/");
    await expect(page.getByRole("heading", { name: "你的 Radar Brief" })).toBeVisible();
    await expect(page.getByText("还没有 Radar Brief")).toBeVisible();

    await page.getByLabel("Brief 名称").fill("Agent 工具需求");
    await page
    ...
    await page.getByLabel("Brief 名称").fill("本地优先产品");
    await page
      .getByLabel("Radar Brief")
    ...
    await expect(page.getByText("关注用户真正拥有数据和运行权的本地优先软件实践。"))
      .toBeVisible();
    await page.getByRole("link", { name: "Radar Brief", exact: true }).click();
    await page.getByRole("link", { name: /Agent 工具需求/ }).click();
    await expect(page.getByText("持续判断独立开发者在构建 AI Agent 时反复遇到的工具缺口。"))
      .toBeVisible();
  } finally {
