...
test("用户可以验证、采集、复用并停止公开 RSS 来源", async ({ browser }) => {
  ...
  try {
    await page.goto("/");
    await page.getByLabel("Brief 名称").fill("来源能力观察");
    ...
    await expect(page.locator(".source-notice")).toHaveText("采集成功，Feed 当前没有来源内容");
    await expect(page.getByText("0 份来源内容", { exact: true })).toBeVisible();
    ...
    await expect(page.locator(".source-notice")).toHaveText("本次新增 2 份来源内容");
    ...
    await page.getByRole("button", { name: "采集 Fallback Identity Feed" }).click();
    await expect(page.locator(".source-notice")).toHaveText("未发现内容变化，复用 2 份来源内容");
    ...
    await expect(page.locator(".brief-source-list")).toHaveAttribute("aria-busy", "true");
    ...
    await expect(radarSource.getByText("1 份来源内容", { exact: true })).toBeVisible();

    await page.getByRole("button", { name: "采集 Radar Fixture Feed" }).click();
    await expect(page.locator(".source-notice")).toHaveText("未发现内容变化，复用 1 份来源内容");
    await expect(radarSource.getByRole("link", { name: "Local-first tools gain traction" })).toHaveCount(1);

    feed.publishChangedEntry();
    await page.getByRole("button", { name: "采集 Radar Fixture Feed" }).click();
    await expect(radarSource.getByText("Local-first tools become inspectable")).toBeVisible();
    await expect(radarSource.getByText("2 份来源内容", { exact: true })).toBeVisible();
    await expect(radarSource.getByText("Local-first tools gain traction")).toBeVisible();
    ...
    feed.breakFeed();
    await page.getByRole("button", { name: "采集 Radar Fixture Feed" }).click();
    await expect(page.locator(".source-notice")).toContainText("Feed XML 无效");
    await expect(radarSource.getByText("2 份来源内容", { exact: true })).toBeVisible();
    await expect(radarSource.getByText("Local-first tools gain traction")).toBeVisible();

    await page.getByRole("button", { name: "停止使用 Radar Fixture Feed" }).click();
    await expect(page.locator(".source-notice")).toHaveText("已停止后续采集，已取得的来源内容保留");
    await expect(radarSource.getByText("2 份来源内容", { exact: true })).toBeVisible();
    await page.getByRole("button", { name: "重新接入 Radar Fixture Feed" }).click();
    await expect(page.locator(".source-notice")).toHaveText(
      "已从本地实例复用 Radar Fixture Feed 和 2 份来源内容；没有重新取得内容。",
    );
    ...
    await expect(page.getByText(
      "重新接入已停止来源，即可恢复手动采集；也可以验证新的公开 URL。已取得的来源内容继续保留。",
    )).toBeVisible();
    ...
    await expect(radarSourceAfterRestart.getByText("已停止后续采集，已取得的来源内容保留")).toBeVisible();
    await expect(radarSourceAfterRestart.getByText("2 份来源内容", { exact: true })).toBeVisible();
